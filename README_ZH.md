...
在上游全部能力之外，本构建额外增加了四项功能，聚焦于**让第三方与推理模型供应商经由本地代理真正可用**：

- **本地网关（`/gateway/*`）** —— 额外暴露一组带鉴权的端点，让 **CC Switch 不接管**的第三方工具也能经由你已配置的供应商转发。每个命名空间（Claude、Codex、Gemini、Grok Build）各自选择路由模式：**model 目录**（只转发你列出的 model，其余一律 404），或**整条转发给一个供应商**（任意 model 名原样送达，不查目录、不 404）。请求须携带 Bearer 令牌，可自行查看、轮换或自定义。与网关的整体设计一致，它**绝不改写任何 CLI 的 Live 配置文件，也绝不产生接管备份行**——纯增量。

- **强制转发推理强度（逐供应商）** —— 默认情况下，请求的推理 `effort` 只有在模型命中内置白名单（o 系列 / gpt-5+ / grok-4.5）时才转发给上游；DeepSeek-R1、Qwen3 一类会被静默丢弃。开启后，只要请求本身带 `effort` 就一律注入上游，不看模型名——适用于接受该字段但不在名单里的上游。

- **system 消息上提到开头（逐供应商）** —— 部分严格的上游会拒收出现在对话中间的 system 消息（报错 `"system message must be at the beginning"`）。开启后把对话中间的 system 消息合并进开头那条，让这类供应商能正常接收。（代价：若被注入的文本每轮都在变，前缀缓存就会每轮未命中——对缓存友好的上游请保持关闭。）

- **内存内请求调试抓取** —— 拨动开关即可把请求体、非流式响应体与错误响应体（如 HTTP 400）留在内存里排查问题。**不落盘，重启即清空**；流式响应只记录 HTTP 状态码。主从式查看器按请求归组，可按通道（客户端 / 上游 / 响应 / 错误）筛选。

> 以上是与上游的差异。以下内容是 CC Switch 的完整功能集。
...
- **本地网关**（`/gateway/*`）— 供 CC Switch 不接管的第三方工具接入的带鉴权端点；每命名空间可选 model 目录模式或整条转发给单一供应商，令牌可查看/轮换/自定义，不改写任何 CLI Live 配置
- **强制转发推理强度**（逐供应商）— 让不在内置白名单里的模型（DeepSeek-R1、Qwen3 等）也能把 `effort` 带给上游
- **system 消息上提到开头**（逐供应商）— 兼容拒收对话中间 system 消息的严格上游
- **内存内请求调试抓取** — 请求/响应/错误体只留内存、不落盘、重启即清空，用于排查上游 4xx
