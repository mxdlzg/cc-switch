...
上流の全機能に加えて、このビルドでは**サードパーティ製・推論モデルプロバイダをローカルプロキシ経由で実際に動作させる**ことに特化した 4 つの機能を追加しています：

- **ローカルゲートウェイ（`/gateway/*`）** —— CC Switch が管理（テイクオーバー）しないサードパーティツールでも、設定済みプロバイダ経由で転送できる認証付きエンドポイント群を公開します。各名前空間（Claude・Codex・Gemini・Grok Build）ごとにルーティングモードを選択：**モデルカタログ**（列挙したモデルのみ転送、それ以外は 404）または**1 プロバイダへ全転送**（任意のモデル名をそのまま透過、カタログ参照なし・404 なし）。リクエストは Bearer トークンで保護され、表示・ローテーション・独自設定が可能です。設計全体の一貫性として、**いずれの CLI ライブ設定も書き換えず、テイクオーバーのバックアップも生成しません** —— 純粋に追加される機能です。

- **推論 effort を強制転送（プロバイダ単位）** —— 既定では、リクエストの推論 `effort` は内蔵の許可リスト（o 系 / gpt-5+ / grok-4.5）にモデルが該当する場合のみ上流へ転送され、DeepSeek-R1 や Qwen3 などは静かに破棄されます。このスイッチを有効にすると、モデル名に関係なく、リクエストが `effort` を運んでいれば常に上流へ注入されます——リスト外でもこのフィールドを受け付ける上流向けです。

- **system メッセージを先頭へ移動（プロバイダ単位）** —— 会話の途中に現れた system メッセージを拒否する厳格な上流（エラー `"system message must be at the beginning"`）に対応します。会話途中の system メッセージを先頭の 1 件に統合し、そうした上流がリクエストを受け付けられるようにします。（トレードオフ：注入されるテキストが毎ターン変わると、プレフィックスキャッシュが毎ターンミスを起こします——キャッシュに優しい上流ではオフにしてください。）

- **メモリ内リクエストデバッグキャプチャ** —— スイッチを入れると、リクエストボディ・非ストリーミングのレスポンスボディ・エラーボディ（HTTP 400 など）をメモリ内に保持して問題切り分けできます。**ディスクには一切書き込まず、再起動で消去**、ストリーミングレスポンスは HTTP ステータスのみ記録します。マスターディテールのビューアでリクエスト単位にグルーピングし、チャネル（クライアント / 上流 / レスポンス / エラー）で絞り込めます。

> 以上は上流との差分です。以下は CC Switch の完全な機能一覧です。
...
- **ローカルゲートウェイ**（`/gateway/*`）— CC Switch が管理しないサードパーティツール向けの認証付きエンドポイント。名前空間ごとにモデルカタログ、または 1 プロバイダへ全転送のモードを選択。トークンは表示／ローテーション／独自設定可。いずれの CLI ライブ設定も書き換えません
- **推論 effort を強制転送**（プロバイダ単位）— 内蔵の許可リスト外のモデル（DeepSeek-R1、Qwen3 など）でも `effort` を上流へ転送
- **system メッセージを先頭へ移動**（プロバイダ単位）— 会話途中の system メッセージを拒否する厳格な上流に対応
- **メモリ内リクエストデバッグキャプチャ** — リクエスト／レスポンス／エラーボディをメモリ内のみ保持、ディスク非書き込み、再起動で消去。上流の 4xx 診断用
